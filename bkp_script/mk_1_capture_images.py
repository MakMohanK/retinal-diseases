...
# ========================================
# CAPTURE AND STACK IMAGES HORIZONTALLY
# ========================================
def stack_horizontally(frame1, frame2):
    # Resize so heights match
    h1, w1 = frame1.shape[:2]
    h2, w2 = frame2.shape[:2]
    target_h = min(h1, h2)
    # scale both frames to the same height
    f1 = cv2.resize(frame1, (int(w1 * target_h / h1), target_h))
    f2 = cv2.resize(frame2, (int(w2 * target_h / h2), target_h))
    # now stack
...
def capture_frames(ir_cam, hd_cam, count=10):
    ...
    for i in range(count):
        ...
        show_image = stack_horizontally(hd, ir)
        cv2.imshow("HD + IR", show_image)
        cv2.waitKey(1)
